...
import crashBetsDb from '../../../db/roulette.js';
...
export async function POST(req, res) {
    if (req.method === 'POST') {
        ...
        console.log(betAmount, gameid,winbet,type,"bet");
        ...
        try {
            ...
            try {
                ...
                crashBetsDb.prepare('INSERT INTO roul (userId, betAmount, gameid,bet,type) VALUES (?, ?, ?,?,?)')
                    ...
                    const res2 = await fetch(process.env.url+'/api/transaction', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ user: userr.username, amount: betAmount, type: 'withdraw',remarks:'Roulette Bet' })
