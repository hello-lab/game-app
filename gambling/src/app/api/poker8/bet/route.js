    ...
    if (req.method === 'POST') {
        ...
        try {
            ...
            try {
                ...
                crashBetsDb.prepare('INSERT INTO poker1 (userId, betAmount, gameid,bet,type) VALUES (?, ?, ?,?,?)')
                    ...
                    const res2 = await fetch(process.env.url+'/api/transaction', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ user: userr.username, amount: betAmount, type: 'withdraw',remarks:'Poker1 Bet' })
