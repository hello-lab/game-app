...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket("ws://localhost:8080?subdomain=poker6");
    setSocket(socket);
  ...
  const handlePlaceBetW = async () => {
    ...
    if (betAmountW > 0) {
      ...
      const res = await fetch("/api/poker6/bet", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div>
      <h1 className="highlights">Baccarat 32 cards</h1>
      <br></br>
      <div className="par1">
        <div className="canvas1" style={{ width: "100%", overflow: "hidden" }}>
          ...
          <div className="winning-hand">
            <h2>Winning Hand:</h2>
            {["PLAYER A     ", "BANKER     ", "Betting Stage","TIE"][winningHand]}
          </div>
        </div>
