...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket("ws://localhost:8080?subdomain=poker2");
    setSocket(socket);
  ...
  return (
    <div>
      <h1 className="highlights">3 CARDS POKER (JOKER)</h1>
      <br></br>
      <div className="par1">
          ...
          {cards.length === 0 ? (
            ...
            cards.map((hand, handIndex) => (
              <div key={handIndex} className="hand">
                <p className="player">{["JOKER","PLAYER A", "PLAYER B"][handIndex]}</p>
