...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket("ws://localhost:8080?subdomain=poker5");
    setSocket(socket);
    ...
    socket.onmessage = (event) => {
      ...
      if (evennt.type === "deal") {
        ...
        setIsCrashed(false);
        if (evennt.hands[0].length==2&&evennt.hands[1].length==2){
          console.log([[...evennt.hands[0],"back.png"],[...evennt.hands[1],"back.png"]])
          setCards([[...evennt.hands[0],"back.png"],[...evennt.hands[1],"back.png"]])
        }
        else if(evennt.hands[0].length==2){

          setCards([[...evennt.hands[0],"back.png"],[...evennt.hands[1]]])
          console.log(cards)
        }
        else if(evennt.hands[1].length==2){
          setCards([[...evennt.hands[0]],[...evennt.hands[1],"back.png"]])
          console.log(cards)
        }
        else
  ...
  const handlePlaceBetW = async () => {
    ...
    if (betAmountW > 0) {
      ...
      const res = await fetch("/api/poker5/bet", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div>
      <h1 className="highlights">Baccarat Super 6</h1>
      <br></br>
      <div className="par1">
        <div className="canvas1" style={{ width: "100%", overflow: "hidden" }}>
          {cards.length === 0 ? (
            ...
            cards.map((hand, handIndex) => (
              <div key={handIndex} className="hand">
                <p className="player">{["PLAYER A", "BANKER"][handIndex]}</p>
          ...
          <div className="winning-hand">
            <h2>Winning Hand:</h2>
            {["PLAYER A     ", "BANKER     ", "Betting Stage","TIE","PLAYER A (S6) ","BANKER (S6) "][winningHand]}
          </div>
        </div>
        ...
        <div>
          ...
          <br></br>
          <div className="Winning-Hand ">
            <div>Super 6 Bet: 2x</div>

            <div className="probablity">
              <p
                className={` ${winbetW == "D" ? "active" : "inactive"}`}
                onClick={() => handleclik("D", "S")}
              >
                A
              </p>
              <p
                className={` ${winbetW == "E" ? "active" : "inactive"}`}
                onClick={() => handleclik("E", "S")}
              >
                Banker
              </p>
