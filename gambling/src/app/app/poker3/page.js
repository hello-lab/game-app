...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket("ws://localhost:8080?subdomain=poker3");
    setSocket(socket);
  ...
  const handlePlaceBetW = async () => {
    ...
    if (betAmountW > 0) {
      ...
      const res = await fetch("/api/poker3/bet", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handlePlaceBetS = async () => {
    ...
    if (betAmountS > 0) {
      ...
      const res = await fetch("/api/poker2/bet", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div>
      <h1 className="highlights">3 CARDS POKER (MUFLIS)</h1>
      <br></br>
      <div className="par1">
